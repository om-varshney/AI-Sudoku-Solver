"""
In this file we aim to make the functions which will be able to take a sudoku image, process it and return the
coordinates of the sudoku grid and also all the cells which are there in the grid.
"""
import math
import cv2
import numpy as np
...
OCR_MODEL = tf.keras.models.load_model("Models/ocr_pro.h5")
plt.gray()
...
def find_cell_values(cells):
    """
    Using an OCR DCN to find out the cell values.
    """
    cells_with_value = []
    for cell in cells:
        plt.imshow(cell['img'])
        plt.show()
        y_pred = np.argmax(OCR_MODEL.predict([(cell["img"] / 255).reshape(1, 28, 28, 1)]))
        print(f"Cell Value Recognised at {cell['pos']} :: Value: {y_pred}")
...
if __name__ == "__main__":
    for file_path in glob.glob("In/*.jpg"):
        ...
        if is_grid_solved:
            with open("sudoku_grid_sol.txt", "a+") as f:
                sudoku_ans = ""
                for i in range(9):
                    if i % 3 == 0 and 0 < i < 9:
                        sudoku_ans += "-" * 21 + "\n"
                    for j in range(9):
                        if j % 3 == 0 and 0 < j < 9:
                            sudoku_ans += "| "
                        sudoku_ans += str(int(grid[i, j])) + " "
                    sudoku_ans += "\n"
                f.write(sudoku_ans)
                f.write("#" * 30)
            print(sudoku_ans)
        else:
            print("Grid is unsolvable: model misread digits")
    print("Program done!")
